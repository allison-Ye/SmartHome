...
void Write_MFRC522(uchar addr, uchar val)
{
    ...
    spidev->tx_buffer[0] = buf[0] ;
    spidev->tx_buffer[1] = buf[1] ;
    spidev_sync_write(spidev, 2);
}
...
uchar Read_MFRC522(uchar addr)
{
    ...
    *spidev->tx_buffer = ((addr<<1)&0x7E) | 0x80;
    spidev_sync_write(spidev, sizeof(uchar));
    *spidev->tx_buffer = 0;
    spidev_sync_write(spidev, sizeof(uchar));
    val = *spidev->rx_buffer;
    return val; 
}
...
spidev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case SPI_IOC_WR_MAX_SPEED_HZ:
        ...
        case SPI_IOC_RD_REG:
        {
            uchar buf[2]; //0:addr 1: val
            if(copy_from_user(buf,(uchar *)arg,2))
                return -EFAULT;
            buf[1] = Read_MFRC522(buf[0]); 
            if(copy_to_user((uchar*)arg + 1, &buf[1],1))
                return -EFAULT;
