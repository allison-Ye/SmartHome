	...
	struct snd_pcm_hw_params *params)
{
	...
	struct snd_soc_dai *codec_dai = rtd->dai->codec_dai;
        ...
        rclk = params_rate(params) * rfs;// psr div


       printk("########## rclk is  %d        bfs is %d     rfs is %d\n",rclk,bfs,rfs);
        ...
        ret = snd_soc_dai_set_sysclk( cpu_dai, S3C_CLKSRC_CLKAUDIO, rate, SND_SOC_CLOCK_OUT );    //66.7MHz
        if( ret < 0 ){
            printk( "%s: AP sysclk S3C_CLKSRC_CLKAUDIO setting error, %d\n", __func__, ret );
            return ret;
        }
        ...
        //printk("PRESCALER=%d ,%s %s:%d\n",psr-1,__func__,__FILE__,__LINE__);
        if (ret < 0)
                return ret;

        ret = snd_soc_dai_set_clkdiv(cpu_dai, S3C_I2SV2_DIV_BCLK, bfs);
        //printk("BCLK=%d ,%s %s:%d\n",bfs,__func__,__FILE__,__LINE__);
        if (ret < 0)
                return ret;

        ret = snd_soc_dai_set_clkdiv(cpu_dai, S3C_I2SV2_DIV_RCLK, rfs);
       //printk("RCLK=%d ,%s %s:%d\n",rfs,__func__,__FILE__,__LINE__);
        if (ret < 0)
                return ret;
...
static int smdk64xx_wm8960_init_paiftx(struct snd_soc_codec *codec)
{
	 ...
	 */
   //snd_soc_dapm_disable_pin(codec, "Line Input 3 (FM)");
...
static int smdk64xx_wm8960_init_paifrx(struct snd_soc_codec *codec)
{
	...
	snd_soc_dapm_add_routes(codec, audio_map_rx, ARRAY_SIZE(audio_map_rx));
        ...
        snd_soc_dapm_enable_pin(codec, "Speaker_L");
        snd_soc_dapm_enable_pin(codec, "Speaker_R");
