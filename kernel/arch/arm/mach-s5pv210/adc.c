...
static irqreturn_t adcdone_int_handler(int irq, void *dev_id)
{
	if (__ADC_locked) {
		...
		__raw_writel(0x0, base_addr + S3C_ADCCLRINT);
                //printk("print in adcdone_int_handler __ADC_locked by adc driver\n");
	}
...
static int __devinit s3c_adc_probe(struct platform_device *pdev)
{
	...
	if (ret) {
		printk(KERN_ERR "cannot request_irq on minor=%d (%d)\n",
			ADC_MINOR, ret);
		goto err_clk;
	}
