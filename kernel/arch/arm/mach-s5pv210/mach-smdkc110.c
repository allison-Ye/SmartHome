...
#include <mach/fsl_devicesplat.h>//blueskyforlinx
...
#ifdef CONFIG_CAMERA_OV9650 //bs2013-10-11
...
static struct gpio_keys_button gpio_buttons[] = {
	...
	{
		.gpio		= S5PV210_GPH0(6),//k4
		.code		= 116, //xyz changed 20130619 15,   
		.desc		= "POWER",//xyz changed 20130619 "VOLUME_UP",	
		.active_low	= 1,
		.wakeup		= 1,
	},
	{
		.gpio		= S5PV210_GPH0(7),//k5
		.code		= 127,//xyz changed 20130619 158,  107 can search
		.desc		= "SEARCH",//xyz changed 20130619 "BACK",	
		.active_low	= 1,
		.wakeup		= 1,
	},
...
#else
#if defined(CONFIG_KEYPAD_S3C_FORLINX)
void s3c_setup_keypad_cfg_gpio_(int rows, int columns)
{
	...
	s3c_gpio_cfgpin(S5PV210_GPJ1(5), S3C_GPIO_SFN(3));
	s3c_gpio_setpull(S5PV210_GPJ1(5), S3C_GPIO_PULL_NONE);
#if 1//defined(!CONFIG_ANDROID) maybe have bug???bs
	end = S5PV210_GPH3(0 + 8);
...
EXPORT_SYMBOL(s3c_setup_keypad_cfg_gpio_);
#else
...
static int lte480wv_backlight_on(struct platform_device *pdev)
{
    ...
    s3c_gpio_cfgpin(S5PV210_GPD0(1), S5PV210_GPD_0_1_TOUT_1);//forlinx change  S5PV210_GPD_0_0_TOUT_0 to S5PV210_GPD_0_1_TOUT_1
...
static struct spi_board_info s3c_spi_devs[] __initdata = {
	[0] = {
		.modalias        = "RC522",	/* device node name */
...
static struct platform_device smdk_backlight_device = {
	...
	.id        = -1,
	.dev        = {
		.parent = &s3c_device_timer[1].dev,//&s3c_device_timer[0].dev,
...
static void __init smdk_backlight_register(void)
{
	...
	for (i = 1; i < ntimer; i++) { //forlinx change i=0 to i=1;
...
static int tvp5150_power(int onoff)//low_reset GPE2_6
{
	int err;	
	err = gpio_request(S5PV210_GPH2(6), "GPH2_6");//zdg
	if (err)
		printk(KERN_ERR "#### failed to request GPH2_6 for RESET\n");	

	s3c_gpio_setpull(S5PV210_GPH2(6), S3C_GPIO_PULL_NONE);
	gpio_direction_output(S5PV210_GPH2(6), 0);
	mdelay(1);
	gpio_direction_output(S5PV210_GPH2(6), 1);
...
static int smdkv210_OV3640_power(int onoff)//low_reset GPH2_6
{
	int err;
	err = gpio_request(S5PV210_GPH2(6), "GPH2_6");//zdg
	if (err)
...
static int smdkv210_ov9650_power_en(int onoff)
{
    int err;
    unsigned int gpio=0;//
#if 0
...
#define S5PV210_GPH0DRV (S5PV210_GPH0_BASE + 0x0C)
    ...
    unsigned tmp=0;
    tmp=readl(S5PV210_GPH0CON);//GPH con
    printk(" before gph0con=%x \n",tmp);
    tmp&=((~0xf)<<0);
    tmp|=(1<<0);
    //writel(tmp,S5PV210_GPH0CON); //output model
    printk(" gph0con=%x \n",tmp);

    tmp=readl(S5PV210_GPH0DAT);//GPH con
    printk("before gph0dat=%x \n",tmp);
    tmp&=((~0x1)<<0);
    tmp|=(0<<0);
    writel(tmp,S5PV210_GPH0DAT); //gph dat
    printk(" gph0dat=%x \n",tmp);
    tmp=readl(S5PV210_GPH0DAT);//GPH con
    printk("before gph0dat=%x \n",tmp);

#endif
	printk(" %s:%d ov9650: power %s\n",__func__,__LINE__, onoff ? "ON" : "Off");
	return 0;
}
...
static struct s3c_platform_camera ov9650 = {
	...
	.initialized	= 0,
    .cam_power	= smdkv210_ov9650_power_en,
};
#endif
...
//forlinx.bs
void gpio_mma7660_get(void)
{
    unsigned int gpio=0;
#if CONFIG_OK210_BOARD_V2
    gpio=S5PV210_GPH0(2);//IRQ_EINT(2)
#else
    gpio=S5PV210_GPH3(2);//IRQ_EINT(26)	 
#endif

    if (!gpio_request(gpio, "mma7660"))
    {
    ...
    {
        printk("mma7660 gpio set error!\n");
    }
    gpio_free(gpio);

}
void gpio_mma7660_put(void)
{
}
...
static struct i2c_board_info i2c_devs0[] __initdata = {
	...
	#endif 
        #ifdef CONFIG_AXIS_MOTION_DETECTION_SENSOR
   	{
	...
	#endif
#endif
        /*cloud sail 2014-2-12*/
        {
            I2C_BOARD_INFO("GY-30", 0x23),
        },
        /*cloud sail 2014-2-12*/
        /*cloud sail 2014-2-12*/
        {
...
{
#ifdef CONFIG_OK210_BOARD_V2
       unsigned int gpio=S5PV210_GPH0(1);
#else
...
#ifdef CONFIG_OK210_BOARD_V2
	#ifdef CONFIG_S3C_DEV_HSMMC3//forlinx bs
	static int sido_reset_wifi(void)//forlinx bs
	{
		int err;
        err = gpio_request(S5PV210_GPH2(2), "GPH2");
		if (err) {
			printk(KERN_ERR "failed to request GPH2 for "
					"sdio_wifi reset control\n");
			return err;
		}
		...
		//printk("call sido_reset_wifi ok\n");//debug
		return 0;
	}//forlinx bs
	#endif //forlinx bs
#else
	#ifdef CONFIG_S3C_DEV_HSMMC2//forlinx bs
	static int sido_reset_wifi(void)//forlinx bs
	{
		int err;
		err = gpio_request(S5PV210_GPH2(7), "GPH2");
		if (err) {
			printk(KERN_ERR "failed to request GPH2 for "
					"sdio_wifi reset control\n");
			return err;
...
static void __init smdkc110_machine_init(void)
{
	...
	/* spi */
#ifdef CONFIG_S3C64XX_DEV_SPI
    printk(KERN_WARNING"S5PV210 SPI INIT!!!!!!!!!!!!!!!!!!!!s\n");
