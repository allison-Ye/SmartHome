...
//parameter
static inline int ov9650_i2c_read(struct v4l2_subdev *sd)//, u8 addr, u8 val)
{
    ...
    int retry = 0;
    int ret=0;
    memset(buf,0x0,sizeof(u8)*2);
    
    if (!client->adapter)
        return -ENODEV;
    //buf[0]=addr;// register address in the chip
    buf[0]=0x0a;
				...
				unsigned char length)
{
    ...
    //dev_dbg(client->dev,"%s:%s:%d ret=%d \n",__FILE__,__func__,__LINE__,ret);
    return ret == 1 ? 0 : -EIO;
}
...
//debug
#if 0
	err=ov9650_i2c_read(sd);
	u8 reg1=0x0a;
	int value1=0;//
	value1=i2c_smbus_read_byte_data(client,reg1);
	printk("value1=%x \n",value1);
#endif
//debug
#if 1
	for (i = 0; i < OV965X_INIT_REGS; i++) {
		if(ov965x_init_reg[i][0]==CHIP_DELAY)
		{
			mdelay(ov965x_init_reg[i][1]);
			continue;
		}
