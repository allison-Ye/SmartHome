...
void fimc_clk_en(struct fimc_control *ctrl, bool on)
{
	...
	pdata = to_fimc_plat(ctrl->dev);
        //printk("before clk_get pdev->dev, pdata->lclk_name='%s' \n",pdata->lclk_name);//debug bs
	lclk = clk_get(&pdev->dev, pdata->lclk_name);
        //printk("&pdev->dev->name=%s, pdata->lclk_name=%s\n",pdev->name,pdata->lclk_name);
        //printk("after clk_get pdev->dev, pdata->lclk_name lclk=%d  on=%d \n",lclk,on);//debug bs 
        //printk("  (%d) Clock %s(%d) enabled.\n",ctrl->id, ctrl->clk->name,ctrl->clk->id);//debug bs
        //printk("clk->rate=%d clk->usage=%d clk->id=%d \n",lclk->rate,lclk->usage,lclk->id);//debug bs
	if (on) {
		if (!lclk->usage) {
			if (!ctrl->out)
