...
static int __init s3c_ts_probe(struct platform_device *pdev)
{
    ...
    int ret, size, err;
    int irq_flags = 0;
    unsigned int x_coor_max=800;//
    unsigned int y_coor_max=480;//
...
//
#ifdef CONFIG_ANDROID
  	if(lcdsize ==1)//480X272
  	{
		x_coor_max=480;//480X272
		y_coor_max=272;//480X272
	}
	else if(lcdsize == 2||lcdsize == 3)
	{
		x_coor_max=800;//800x480
		y_coor_max=480;//800x480
	}
	else if(lcdsize == 4)
	{
		x_coor_max=800;////800x600
        y_coor_max=600;////800x600	
	}
	else
	{
		x_coor_max=480;//480X272
		y_coor_max=272;//480X272;////480X272
        }   
		printk("x_coor_max=%d ,y_coor_max=%d \n",x_coor_max,y_coor_max);
		input_set_abs_params(ts->dev, ABS_X, 0, x_coor_max, 0, 0);//input_set_abs_params(ts->dev, ABS_X, 0, 800, 0, 0);
		input_set_abs_params(ts->dev, ABS_Y, 0, y_coor_max, 0, 0);//input_set_abs_params(ts->dev, ABS_Y, 0, 480, 0, 0);
