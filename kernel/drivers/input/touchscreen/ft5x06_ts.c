...
FTS_BOOL i2c_read_interface(FTS_BYTE bt_ctpm_addr, FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    if(ret<=0)
    {   
        if(TS_DEBUG)   printk("[FTS]i2c_read_interface error\n");
        return FTS_FALSE;
    }
...
FTS_BOOL i2c_write_interface(FTS_BYTE bt_ctpm_addr, FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    if(ret<=0)
    {
        if(TS_DEBUG) printk("[FTS]i2c_write_interface error line = %d, ret = %d\n", __LINE__, ret);
        return FTS_FALSE;
    }
...
E_UPGRADE_ERR_TYPE  fts_ctpm_fw_upgrade(FTS_BYTE* pbt_buf, FTS_DWRD dw_lenth)
{
    ...
    if(TS_DEBUG) printk("[FTS] Step 1: Reset CTPM test\n");
    ...
    {
        if(TS_DEBUG) printk("[FTS] Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",reg_val[0],reg_val[1]);
    }
    else
    {
        return ERR_READID;
    ...
    byte_read(reg_val,1);
    if(TS_DEBUG) printk("[FTS] bootloader version = 0x%x\n", reg_val[0]);
    ...
    delay_qt_ms(100);
    if(TS_DEBUG) printk("[FTS] Step 4: erase. \n");

    /*********Step 5:write firmware(FW) to ctpm flash*********/
    bt_ecc = 0;
    if(TS_DEBUG)   printk("[FTS] Step 5: start upgrade. \n");
    dw_lenth = dw_lenth - 8;
    ...
    for (j=0;j<packet_number;j++)
    {
        ...
        {
              if(TS_DEBUG) printk("[FTS] upgrade the 0x%x th byte.\n", ((unsigned int)j) * FTS_PACKET_LENGTH);
        }
    }
    ...
    byte_read(reg_val,1);
    if(TS_DEBUG)   printk("[FTS] Step 6:  ecc read 0x%x, new firmware 0x%x. \n", reg_val[0], bt_ecc);
    if(reg_val[0] != bt_ecc)
    {
        return ERR_ECC;
    }
...
static int fts_Get_RawData(void)
{
	...
	if(TS_DEBUG) pr_info("Read rawdata .......\n");
...
int fts_ctpm_auto_clb(void)
{
    ...
    if(TS_DEBUG)   printk("[FTS] start auto CLB.\n");
    msleep(200);
    ...
    for(i=0;i<100;i++)
    {
        ...
        delay_qt_ms(200);
        if(TS_DEBUG) printk("[FTS] waiting calibration %d\n",i);
        
    }
    if(TS_DEBUG) printk("[FTS] calibration OK.\n");
    ...
    msleep(300);
    if(TS_DEBUG) printk("[FTS] store CLB result OK.\n");
    return 0;
}
...
int fts_ctpm_fw_upgrade_with_i_file(void)
{
   ...
   if (i_ret != 0)
   {
       if(TS_DEBUG) printk("[FTS] upgrade failed i_ret = %d.\n", i_ret);
       //error handling ...
       //TBD
   }
   else
   {
       if(TS_DEBUG) printk("[FTS] upgrade successfully.\n");
...
int fts_ctpm_update_project_setting(void)
{
    ...
    cmd_write(0xcd,0x0,0x00,0x00,1);
    byte_read(reg_val,1);
    if(TS_DEBUG) printk("bootloader version = 0x%x\n", reg_val[0]);
    ...
    if(TS_DEBUG) printk("[FTS] old setting: uc_i2c_addr = 0x%x, uc_io_voltage = %d, uc_panel_factory_id = 0x%x\n",
        buf[0],  buf[2], buf[4]);
    if(TS_DEBUG) {
    ...
    for (i = 0; i < FTS_SETTING_BUF_LEN; i++)
    {
        ...
        if(TS_DEBUG)   if (i % 16 == 0)     printk("\n");
        if(TS_DEBUG)   printk("0x%x, ", buf[i]);
    }
    if(TS_DEBUG)   printk("\n");
...
int fts_ctpm_auto_upg(void)
{
    ...
    {
        msleep(100);
        if(TS_DEBUG)   printk("[FTS] uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
        ...
        if (i_ret == 0)
        {
            msleep(300);
            uc_host_fm_ver = fts_ctpm_get_i_file_ver();
            if(TS_DEBUG) printk("[FTS] upgrade to new version 0x%x\n", uc_host_fm_ver);
        }
        else
        {
            if(TS_DEBUG) printk("[FTS] upgrade failed ret=%d.\n", i_ret);
        }
    }
...
static int ft5x0x_read_data(void)
{
    ...
    if(TS_DEBUG)   printk("start read_data i2c_rxdata\n");


	ret = ft5x0x_i2c_rxdata(buf, CFG_POINT_READ_BUF);
    if (ret < 0) {
		if(TS_DEBUG)   printk("in %s read_data i2c_rxdata failed: code:%d on line :%d\n", __func__, ret, __LINE__);
		return ret;
	}
    ...
    {
       if(TS_DEBUG)   printk("Touch up,it is end !\n");
    }

    event->pressure = 200;

    if(TS_DEBUG)   printk("start read_data i2c_rxdata OK\n");
...
int ft5x0x_touch_key_process(struct input_dev *dev, int x, int y, int touch_event)
{
    ...
    for(i = 0; i <CFG_NUMOFKEYS; i++ )
    {
        if(tsp_keystatus[i])
        {
            ...
            if(TS_DEBUG)   printk("[FTS] %s key is release. Keycode : %d\n", tsp_keyname[i], tsp_keycodes[i]);
        ...
        else if( key_id == i )
        {
            if( touch_event == 0)                                  // detect
            {
                ...
                if(TS_DEBUG)   printk( "[FTS] %s key is pressed. Keycode : %d\n", tsp_keyname[i], tsp_keycodes[i]);
...
static void ft5x0x_report_value(void)
{
	...
	for (i  = 0; i < event->touch_point; i++)
	{
        ...
        if (event->au16_x[0] < TOUCHSCREEN_MAXX && event->au16_y[0] < TOUCHSCREEN_MAXY)  // LCD view area
        {
            ...
            if(TS_DEBUG)   printk("Linux Touch down,it is start \n");
    ...
    if(event->touch_point==0)//touch up
    {
        ...
        for(k=0;k<CFG_NUMOFKEYS_FORLINX;k++)
        {
            if(tsp_keystatus_forlinx[k] == KEY_PRESS)
            {
                ...
                if(TS_DEBUG)   printk( "[FTS] %s key is up. Keycode : %d\n", tsp_keyname_forlinx[k], tsp_keycodes_forlinx[k]);
            }
        }
#endif
#else  //linux
     ...
     if(TS_DEBUG)   printk("Linux Touch up,it is end \n");
...
//	ts =  container_of(handler, struct ft5x0x_ts_data, early_suspend);
	...
	if(TS_DEBUG)   printk("==ft5x0x_ts_suspend=\n");
...
{
	if(TS_DEBUG)   printk("==ft5x0x_ts_resume=\n");
	// wake the mode
...
static int  ft5x0x_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(TS_DEBUG)   printk("[FTS] ft5x0x_ts_probe, driver version is %s.\n", CFG_FTS_CTP_DRIVER_VERSION);
...
//is ft5x06
       ...
       //uc_reg_value=192 while read iic error 
        uc_reg_value = ft5x0x_read_fw_ver();
	printk("1 uc_reg_value=%d \n",uc_reg_value);
	//if(uc_reg_value!=19)////uc_reg_value=19 while read iic firme ware
	//{
    //   		printk("read iic error\n");		
	//	err = -ENODEV;
	//	goto exit_check_functionality_failed;
	//}//
//is ft5x06
...
#ifdef CONFIG_HAS_EARLYSUSPEND
	if(TS_DEBUG)   printk("==register_early_suspend =\n");
    ...
    //uc_reg_value = ft5x0x_read_fw_ver();
    if(TS_DEBUG)   printk("[FTS] Firmware version = 0x%x\n", uc_reg_value);
    ft5x0x_read_reg(FT5X0X_REG_PERIODACTIVE, &uc_reg_value);
    if(TS_DEBUG)   printk("[FTS] report rate is %dHz.\n", uc_reg_value * 10);
    ft5x0x_read_reg(FT5X0X_REG_THGROUP, &uc_reg_value);
    if(TS_DEBUG)   printk("[FTS] touch threshold is %d.\n", uc_reg_value * 4);
//
#ifdef CONFIG_ANDROID
#else
    //printk("uc_reg_value=%d \n",uc_reg_value);
    // uc_reg_value=128 while read iic error 
    if(uc_reg_value<128&&uc_reg_value>0)
    {
...
//
   ...
   if (0 != err)
  {
    ...
    {
        if(TS_DEBUG)   printk("ft5x0x:%s() - sysfs_create_group() succeeded.\n", __FUNCTION__);
    }
	
	if(TS_DEBUG)   printk("[FTS] ==probe over =\n");
	printk("2 uc_reg_value=%d \n",uc_reg_value);
	if(uc_reg_value>=128)////uc_reg_value=19 while read iic firme ware
	{
       		printk("read iic error\n");		
		//err = -ENODEV;
		disable_irq(reqirq_id);
...
exit_create_singlethread:
	if(TS_DEBUG)   printk("==singlethread error =\n");
...
{
	struct ft5x0x_ts_data *ft5x0x_ts;
	if(TS_DEBUG)   printk("==ft5x0x_ts_remove=\n");
...
{
	int ret;
    if(TS_DEBUG)   printk("ft5x0x_ts_init\n");
	ret = i2c_add_driver(&ft5x0x_ts_driver);
	if(TS_DEBUG)   printk("ret=%d\n",ret);
	return ret;
...
{
	if(TS_DEBUG)   printk("==ft5x0x_ts_exit==\n");
